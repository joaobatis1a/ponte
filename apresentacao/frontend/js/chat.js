...
(() => {
  ...
  // Etapas do diálogo
  const STEPS = ['saudacao', 'nome', 'idade', 'bairro', 'experiencias', 'pergunta_formacao', 'formacao_loop', 'contato', 'fim'];
  let stepIdx = 0;
  ...
  function addRow(side, html, opts = {}) {
    ...
    if (side === 'bot') {
      ...
      av.innerHTML = '<img src="assets/logo-1.png" alt="" />';
      row.appendChild(av);
    }
...
async function extractBairro(cep) {
  ...
  function persist() {
    try {
      ...
      console.log('[Manguelito] persistindo dados no localStorage:', JSON.parse(JSON.stringify(data)));
      ...
      if (data.nome && data.sobrenome) {
        ...
        console.log('[Manguelito] ponte_basico atualizado:', basico);
      }
      if (data.formacoes.length) {
        localStorage.setItem('ponte_formacoes', JSON.stringify(data.formacoes));
        console.log('[Manguelito] ponte_formacoes atualizado:', data.formacoes);
      }
    } catch (e) {
      console.warn('[Manguelito] falha ao persistir localStorage:', e);
    }
  }

  // ---------- Máquina de diálogo ----------
  async function startConversation() {
    console.log('[Manguelito] iniciar conversa');
    setProgress(15);
    await botSayMany([
    ...
    console.log('[Manguelito] próximo step', stepIdx, STEPS[stepIdx]);
  }

  async function handleUserInput(text) {
    if (!text || !text.trim()) return;
    console.groupCollapsed('[Manguelito] entrada de usuário');
    console.log('texto recebido:', text);
    console.log('stepIdx atual:', stepIdx, 'step:', STEPS[stepIdx]);
    console.log('estado atual antes:', JSON.parse(JSON.stringify(data)));
    console.groupEnd();
    ...
    if (step === 'nome') {
      ...
      console.log('[Manguelito] extração nome:', data.nome, data.sobrenome);
      persist();
      setProgress(30);
    ...
    if (step === 'idade') {
      ...
      data.idade = idade;
      console.log('[Manguelito] extração idade:', data.idade);
      persist();
      setProgress(45);
    ...
    if (step === 'experiencias') {
      ...
      if (!exps.length) {
        await botSay('Bora lá, me conta pelo menos uma experiência! Não precisa ser de carteira assinada não.');
        return;
      }
      data.experiencias = exps;
      console.log('[Manguelito] extração experiencias:', data.experiencias);
      setProgress(75);
      await botSay(`Boa! Anotei aqui: ${exps.join(', ')}.`);
      await botSay('Pra eu deixar teu perfil redondo, tu estuda ou já estudou em algum lugar? (escola, curso técnico...)', {
        chips: ['Sim, estudo/estudei', 'Não']
      });
    ...
    if (step === 'pergunta_formacao') {
      if (isNegative(text)) {
        setProgress(85);
        await botSay('Beleza, formação é opcional. Agora me passa teu WhatsApp ou email pra eu te avisar quando o PONTE lançar.');
        stepIdx = STEPS.indexOf('contato');
        console.log('[Manguelito] próximo step', stepIdx, STEPS[stepIdx]);
        return;
      }
    ...
    if (step === 'contato') {
      if(text.length < 5) {
         await botSay('Digita direitinho, pô! Qual teu WhatsApp ou Email?');
         return;
      }
      data.contato = text.trim();
      console.log('[Manguelito] contato capturado:', data.contato);
      persist();
      setProgress(95);
      await botSay(`Fechou! Anotei teu contato: ${data.contato}`);
      await dispararParaOSheets(data);
      await botSay('Tudo salvo, mestre! Criei a base do teu Ponte Pass. Fica de olho no celular que a gente te avisa quando lançar a versão final com as empresas. Tamo junto! 🚀');
    ...
    if (step === 'fim') {
      await botSay('Já fechamos teu cadastro! A gente se fala pelo contato que tu mandou.');
      return;
    }
  ...
  async function finishConversation() {
    if (!data.nome || !data.sobrenome || !data.idade || !data.bairro) {
      await botSay("Ops, faltam dados obrigatórios. Vamos refazer?");
      stepIdx = 1; // Reiniciar
      return;
    }
